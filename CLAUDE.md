...
Check the work, stage only this task's files, review the staged diff, and commit. Integrate completed work into `master` and push `origin/master`; do the same when the organizer asks to save work in progress. This applies even when working on another branch or in a detached worktree. Fetch and safely reconcile any remote changes before pushing; never force-push or sweep up untracked files. In particular, the root `ChatExport_.../` folder may contain private raw data and is not covered by the current ignore rule. If direct publication to `master` is blocked, use a review branch when possible and say explicitly that `master` is not updated. End with a brief, non-technical summary and the human decision or next action, if any.
